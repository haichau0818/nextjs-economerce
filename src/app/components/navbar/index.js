...
import CommonModel from "@/app/CommonModel";
...
export default function Navbar(){
    ...
    return <>
        <nav className="bg-white fixed text-black w-full z-20 top-0 left-0 border-b border-gray-200">
            <div className="max-w-screen-xl flex flex-wrap item-center justify-between mx-auto p-4">
                <div className="flex items-center cursor-pointer">
                    <span className="slef-center text-2xl  font-semibold whitespace-nowrap ">Shop-Web</span>
                </div>
                <div className="flex md:order-2 gap-2">
                    ...
                    {
                        isAuthUser?<button className={"mt-1.5 inline-block bg-black px-5 py-3 text-xs font-medium uppercase tracking-wide text-white"}>Logout</button>:<button className={"mt-1.5 inline-block bg-black px-5 py-3 text-xs font-medium uppercase tracking-wide"}>Login</button>
        ...
        <CommonModel showModelTitle={false} mainContent={<NavItems isModelView={true}/>} show={showNavModel} setShow={setShowNavModal}/>
    </>
}